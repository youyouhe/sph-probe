...
## 6. 服务端缓存 verify 结果：一个真实踩过的坑

verify 端点每次调用都有网络开销，接入方通常会在自己服务端缓存验证结果（sph 缓存 10 分钟），这是合理且推荐的做法。但缓存实现有一个隐蔽的坑，sph 上线当天就踩到了：

**缓存过期 ≠ token 无效。** 错误写法是"命中缓存但发现已过期 → 返回未登录"——这样缓存过期后该 token 会被永久判死（每次请求都命中这条过期缓存、永远不重新 verify），用户表现为"登录成功后过几分钟，刷新就掉登录，且永远登不回来"。

正确写法：缓存过期时**删除该条目并重新调 verify**。一句话原则：

- 缓存有效 → 直接用缓存结果
- 缓存过期 → 删掉，重新 verify（结果可能仍是有效登录态）
- verify 返回 401 → token 确认无效，可短缓存（如 60 秒）防刷，到期后自然重试
- verify 网络异常/4A 不可达 → 建议降级按匿名处理（fail open），不要 500 打挂业务；具体权衡各 app 自行决定

另外强烈建议：verify 失败时在服务端日志记一行原因（401？命中无效缓存？网络错误？）。"用户回跳后掉登录"这类问题，没有这行日志几乎无从排查——sph 排查时靠的就是对比"独立复现 verify 正常、线上却静默返回匿名"才定位到缓存分支。
